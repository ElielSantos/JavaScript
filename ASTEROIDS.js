...
context.fillText("PRESSIONE QUALQUER TECLA PARA INICIAR", canvas.width / 2, canvas.height * 0.58);
...
// DESENHA A NAVE
function drawShip(x, y, a, color = "#00ff0e") { //COR DA NAVE
...
function update () {
	...
	// BACKGROUND
	context.fillStyle = "rgba(44,44,44,1.00)"; //CORES DO FUNDO
	context.fillRect(0, 0, canvas.width, canvas.height);
	
	// DESENHA O ASTEROIDES
	let x, y, r, a, vert, offs;
	for (let i = 0; i < roids.length; i++) {
		context.strokeStyle = "rgba(234, 0, 173, 1)"; //CORES DOS ASTEROIDES
		context.lineWidth = shipSize / 20;
	...
	for (let i = 0; i < ship.lasers.length; i++) {
		if (ship.lasers[i].explodeTime == 0) {
			context.fillStyle = "rgba(255, 125, 0, 1)"; //cores do laser 
			context.beginPath();
	...
	for (let i = 0; i < lives; i++) {
		lifeColors = exploding && i === lives - 1 ? "rosa" : "#ff00f1";
